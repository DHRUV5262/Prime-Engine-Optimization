...
struct ClientGameObjectManagerAddon : public GameObjectManagerAddon
{
	...
	// Event handler for creating tank from Lua meta file
	PE_DECLARE_IMPLEMENT_EVENT_HANDLER_WRAPPER(do_CreateTank);
	virtual void do_CreateTank(PE::Events::Event *pEvt);

	// Legacy hardcoded tank creation (used when not using meta files)
	void createTank(int index, int &threadOwnershipMask);
	
	// Create tank from event data (data-driven)
	void createTankFromEvent(Events::Event_CreateTank *pEvt);
