	...
	if (m_state == WALKING_TO_TARGET || m_state == RUNNING_TO_TARGET)
	{
		...
		if (pSN)
		{
			...
			if (dsqr > 0.01f)
			{
				...
				float speed = (m_state == WALKING_TO_TARGET) ? 1.4f : 2.4f;
				...
				////// Find and update the physics component
				//PhysicsComponent* pPhys = pSN->getFirstComponent<PhysicsComponent>();
				////if (pPhys && !pPhys->m_isStatic) {
				////	pPhys->syncFromSceneNode();
				////}

				//if (pPhys && !pPhys->m_isStatic) {
				//	pPhys->setVelocity(dir * speed); // Set velocity towards target
				//}
			}
